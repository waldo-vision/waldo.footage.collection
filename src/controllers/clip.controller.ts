...
import { Clip, ClipInput, ClipDocument, ClipZod, ClipZodSchema, ClipRetrieveZod, ClipRetrieveSchema } from '../models/clip.interface'
...
export const getClip = defaultEndpointsFactory.build({
  ...
  handler: async ({ input: { uuid }, options, logger }) => {
    ...
    if (uuid) {
      ...
      if (clip === null) {
        console.log("error")
        throw createHttpError(404, 'No clip document with the UUID provided could be found.');
      }
 ...
 */
 export const deleteClip = defaultEndpointsFactory.build({
  ...
  if (deleteResult.deletedCount === 0) {
    throw createHttpError(404, `Couldn't find the clip associated with uuid ${uuid}. This can also occur if the clip has already been deleted.`)
  }
